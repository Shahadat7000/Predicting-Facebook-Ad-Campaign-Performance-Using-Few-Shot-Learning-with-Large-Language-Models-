...
class MultiModelExperiment:
    ...
    def __init__(
        ...
        if self.model_type in {"gpt", "deepseek", "qwen", "cerit"}:
            self.base_url = (
                ...
                or os.getenv("OPENAI_BASE_URL", "https://llm.ai.e-infra.cz/v1")
            )
            ...
            raise ValueError(
                ...
                "Use 'gpt', 'deepseek', 'qwen', or 'cerit'."
            )

        if not self.api_key:
            self.api_key = "dummy_key_for_ceritsc"
            logger.warning(
                "No API key found. Using dummy key. "
                "Set GPT_API_KEY/OPENAI_API_KEY if your endpoint requires it."
            )
        ...
        logger.info(
            f"Initialized MultiModelExperiment with "
            f"model_type={self.model_type}, model={self.model_name}"
        )
    ...
    def parse_prediction(self, response: str) -> int:
        ...
        final_match = re.search(
            r"(?:final\s*answer|answer|prediction)\s*[:\-]?\s*([01])\b",
            text,
            re.IGNORECASE,
        )
        ...
        if (
            "unsuccessful" in lower
            or "not successful" in lower
            or "failed" in lower
        ):
            return 0
        ...
        logger.warning(
            f"Unclear response, defaulting to 0: "
    ...
    def run_experiment(
        ...
        logger.info(
            f"Starting experiment for {self.model_name} "
            f"with shot levels: {shot_levels}"
        )
        ...
        if sample_size is not None:
            ...
            logger.info(f"Using sample of {len(test_data)} test samples")
        else:
            test_labels = test_labels.loc[test_data.index]
        ...
        for n_shots in shot_levels:
            ...
            for i in range(0, len(test_data), batch_size):
                ...
                for _, row in batch.iterrows():
                    try:
                        if n_shots == 0:
                            prompt = prompt_engineer.create_zero_shot_prompt(
                                row,
                                template_type,
                            )
                            examples_used = None
                        ...
                        actual = int(test_labels.loc[row.name])
                        ...
                        actuals.append(
                            int(test_labels.loc[row.name])
                            if row.name in test_labels.index
                            else None
                        )
            ...
            valid_indices = [
                ...
                if pred is not None and actuals[idx] is not None
            ]
            ...
            if valid_predictions:
                ...
                results_by_shot[n_shots] = {
                    ...
                    "n_samples": int(len(valid_predictions)),
                    "n_errors": int(len(errors)),
                    "avg_tokens": (
                        float(np.mean([u["total_tokens"] for u in usage_info]))
                        if usage_info
                        else 0.0
                    ),
                    "avg_response_time": (
                        float(np.mean([u["response_time"] for u in usage_info]))
                        if usage_info
                        else 0.0
                    ),
                    "total_cost": float(self.cost_tracker["total_cost_usd"]),
                }
    ...
    def safe_encode_categorical(
        ...
        for col in categorical_cols:
            ...
            categories = {
                value: idx
                for idx, value in enumerate(sorted(combined_values.unique()))
            }
    ...
    def run_baseline_comparison(
        ...
        X_train_base = X_train_base.drop(
            columns=[c for c in non_feature_cols if c in X_train_base.columns],
            errors="ignore",
        )

        X_test_base = X_test_base.drop(
            columns=[c for c in non_feature_cols if c in X_test_base.columns],
            errors="ignore",
        )
        ...
        for name, model in models.items():
            ...
            logger.info(
                f"{name} baseline: "
                f"accuracy={baseline_results[name]['accuracy']:.4f}, "
    ...
    def save_results(self, filepath: Optional[str] = None) -> str:
        ...
        if filepath is None:
            ...
            filepath = (
                f"results/{self.model_type}_"
                f"{safe_model_name}_{timestamp}_results.json"
            )
    ...
    def generate_cost_report(self) -> Dict:
        ...
        report = {
            "model_type": self.model_type,
            "model_name": self.model_name,
            "total_api_calls": int(self.cost_tracker["api_calls"]),
            "failed_calls": int(self.cost_tracker["failed_calls"]),
            "success_rate": float(
                (
                    self.cost_tracker["api_calls"]
                    - self.cost_tracker["failed_calls"]
                )
                / total_calls
                * 100
            ),
            "total_prompt_tokens": int(self.cost_tracker["total_prompt_tokens"]),
            "total_completion_tokens": int(
                self.cost_tracker["total_completion_tokens"]
            ),
            "total_tokens": int(total_tokens),
            "total_cost_usd": float(self.cost_tracker["total_cost_usd"]),
            "avg_cost_per_call": float(
                self.cost_tracker["total_cost_usd"] / total_calls
            ),
            "avg_tokens_per_call": float(total_tokens / total_calls),
        }
...
def main():
    ...
    sample_size_input = input(
        "Enter number of test samples to use "
        "(recommended >= 100, or 'all' for all): "
    ).strip()
    ...
    if sample_size_input.lower() == "all":
        ...
        try:
            ...
            sample_size = max(1, min(sample_size, total_test))
        except ValueError:
            logger.warning("Invalid sample size input. Using all test samples.")
            sample_size = total_test
